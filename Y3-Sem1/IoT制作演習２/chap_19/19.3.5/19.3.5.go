...
func main() {
	...
	gpioRegister[0] = (gpioRegister[0] &^ (7 << 12)) | (1 << 12) // GPIO4を出力に設定
	for { // 条件を指定しない場合無限ループ
		gpioRegister[7] = 1 << 4    // GPIO4をHighに設定
		fmt.Println("Hiレベル出力") // コンソールにGPIO4の状態を表示
		time.Sleep(2 * time.Second) // 2秒待ち

		gpioRegister[10] = 1 << 4   // GPIO4をLowに設定
		fmt.Println("Loレベル出力") // コンソールにGPIO4の状態を表示
		time.Sleep(2 * time.Second) // 2秒待ち
	}
